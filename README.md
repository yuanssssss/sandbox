...
- 终态事件发出后，流会结束
- execution 和 judge job 两类 SSE 现在共用统一事件模型，都会输出：
  `task_kind`、`event_type`、`status`、`stage`、`message`
...
- execution / judge job 事件流现在复用统一 payload：客户端可按 `task_kind = execution|judge_job`
  和 `event_type = task_status|stage` 做统一处理
...
2. 继续补更细粒度的权限模型，例如按路由区分只读/提交/管理操作
3. 增加更完整的运维配置，例如鉴权轮换、审计留档和限流观测
